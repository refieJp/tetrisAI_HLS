...
{
#pragma HLS function_instantiate variable=batch
...
//==================================================================
    ...
    bool boardArr3[MAX_BATCH][20][MAX_COL];
    bool boardArr4[MAX_BATCH][20][MAX_COL];

    bool rowEliminated[MAX_BATCH][20];
    bool rowEliminated2[MAX_BATCH][20];
    bool rowEliminated3[MAX_BATCH][20];
    bool rowEliminated4[MAX_BATCH][20];
...
#pragma HLS PIPELINE II=240
    	readRow:for (int row = 4, rowIdx = 0; row < MAX_ROW; row++, rowIdx++)
    	{
    		readCol:for (int col = 0; col < MAX_COL; col++)
    		{
    			...
    			boardArr2 [batch][rowIdx][col] = boardArr[batch][rowIdx][col];
                        boardArr3 [batch][rowIdx][col] = boardArr[batch][rowIdx][col];
                        boardArr4 [batch][rowIdx][col] = boardArr[batch][rowIdx][col];

    			rowEliminated[batch][rowIdx] &= boardArr[batch][rowIdx][col];
    			rowEliminated2[batch][rowIdx] = rowEliminated[batch][rowIdx];
                        rowEliminated3[batch][rowIdx] = rowEliminated[batch][rowIdx];
                        rowEliminated4[batch][rowIdx] = rowEliminated[batch][rowIdx];
 		}
    	}
    ...
    batchLoop:for (int ibatch = 0; ibatch < MAX_BATCH/2; ibatch++)
    {
        int batch = ibatch << 1;
        int batch2 = batch + 1;

        rowScores[batch] = getRowScores(boardArr[batch], batch, rowEliminated[batch]);
        rowScores[batch2] = getRowScores(boardArr2[batch2], batch2, rowEliminated2[batch2]);
        colScores[batch]  = getColScores(boardArr3[batch], batch, rowEliminated3[batch]);
        colScores[batch2]  = getColScores(boardArr4[batch2], batch2, rowEliminated4[batch2]);
    }
